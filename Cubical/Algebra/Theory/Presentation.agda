...
-- closed term in a model of σ[V] is the interpretation of the original
...
module _ {σ : AlgTheorySig ℓ ℓv} {V : Type ℓv} (P : Presentation σ V ℓR)
  ...
  -- the relations as equations of σ[V] with no variables
  ...
  module _ (σeq : AlgTheoryEqns σ ℓ'' ℓv) where
    ...
    PresEqns' = σ[V] σeq V ∪Eqns RelEqns
    ...
    PresentedσAlg = forgetPoints σeq V
      (Presented , trunc) (Alg∪ (σ[V] σeq V) RelEqns PresentedAlg)
    ...
    module _ {X : Type ℓX} (isSetX : isSet X) (A : Alg σeq X) where
      ...
      private
        module _ ((ρ , sat) : Points) where
          ...
          N : Alg PresEqns' X
          N = mk∪ (σ[V] σeq V) RelEqns N[V] satRel
