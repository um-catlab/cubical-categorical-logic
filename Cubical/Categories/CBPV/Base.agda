...
record CBPVModel (ℓC ℓC' ℓVTy ℓVTm ℓCTy ℓCTm : Level) : Typeω where 
  field
...
record CBPVModelHom 
    (M : CBPVModel ℓC ℓC' ℓVTy ℓVTm ℓCTy ℓCTm)
    (N : CBPVModel ℓD ℓD' ℓVTy' ℓVTm' ℓCTy' ℓCTm') : Typeω where
  private 
    module M = CBPVModel M 
    module N = CBPVModel N
  field 
    ScwfFun : PreFunctor M.Scwf N.Scwf


  ctxFun = ScwfFun .fst
  ℓS = ℓ-max ℓC (ℓ-max ℓC' (ℓ-max ℓCTm (ℓ-max ℓD (ℓ-max ℓD' ℓCTm'))))
  V = PshMon.𝓟Mon {ℓS = ℓS} M.C
  field 
    F-stack : EnrichedFunctor V {! M.Stacks  !} {!   !} 
...
module _ 
  ...
  record CBPVFunctor : Type _ where 
    ...
    field 
      ...
      F-stack : EnrichedFunctor PMC.𝓟Mon compCatC (BaseChange {! preFun .fst !} {!   !}) 
