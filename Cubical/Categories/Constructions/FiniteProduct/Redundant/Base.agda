...
module Cubical.Categories.Constructions.FiniteProduct.Redundant.Base where
...
ℓs-max (suc n) ℓs = ℓ-max (ℓs-max n (ℓs ∘f fsuc)) (ℓs fzero)

-- data gen
--   (ℓ ℓ' : ℕ → Level)
--   (C : (k : ℕ) → Category (ℓ k) (ℓ' k)) : (n : ℕ) → Type {!!} where

--   hom : ∀ n {c c'} (f : (C n) [ c , c' ]) → {!!} → gen ℓ ℓ' C n

module _ (n : ℕ)
  (ℓ ℓ' : Fin n → Level)
  (C : (k : Fin n) → Category (ℓ k) (ℓ' k)) where

  data gen (j k : Fin n) :
    Type (ℓ-max (ℓ-max (ℓ j) (ℓ' j)) (ℓ-max (ℓ k) (ℓ' k))) where
    hom-j :
      ∀ {cj cj' : C j .ob}
        → (fj : (C j) [ cj , cj' ])
        → (ck : C k .ob)
  ...
  data ax (j k : Fin n) :
    Type (ℓ-max (ℓ-max (ℓ j) (ℓ' j)) (ℓ-max (ℓ k) (ℓ' k))) where
    jk-id : (cj : C j .ob) (ck : C k .ob) → ax j k
    jk-seq :
      ∀ {cj cj' cj'' : C j .ob}{ck ck' ck'' : C k .ob} →
      (fj : (C j) [ cj , cj' ]) (fj' : (C j) [ cj , cj' ]) →
      (fk : (C k) [ ck , ck' ]) (fk' : (C k) [ ck , ck' ]) →
      ax j k
    j-agree :
      ∀ {cj cj' : C j .ob} →
      (fj : (C j) [ cj , cj' ])
...
-- gens :
--   (n : ℕ) →
--   (ℓ ℓ' : Fin (suc (suc n)) → Level) →
--   (C : (k : Fin (suc (suc n))) → Category (ℓ k) (ℓ' k)) →
--   Type (ℓ-max (ℓs-max _ ℓ) (ℓs-max _ ℓ'))
-- gens zero ℓ ℓ' C = ProdGenerator (C fzero) (C (fsuc fzero))
-- gens (suc n) ℓ ℓ' C =
--   -- Use a morphism from generator at earlier stage and keep new ob fixed
--   (Σ[ new-ob ∈ C fzero .ob ]
--     gens n (ℓ ∘f fsuc) (ℓ' ∘f fsuc) (λ k → C (fsuc k)))
--    ⊎
--    -- Or use a new morphism while fixing everything else
--   ( (Σ[ obs ∈ (∀ (k : {!!}) → C (fsuc k) .ob) ] {!!})
--   -- (Σ[ (c , c') ∈ (C fzero .ob) × (C fzero .ob) ]
--   --   (((C fzero) [ c , c' ]) ×
--   --   (∀ (k : Fin n) → C {!!} .ob)))
--   ⊎
--   {!!})
...
-- gens zero ℓ ℓ' C = Unit
-- gens (suc zero) ℓ ℓ' C = Unit*
-- gens (suc (suc zero)) ℓ ℓ' C = ProdGenerator (C fzero) (C (fsuc fzero))
-- gens (suc (suc (suc n))) ℓ ℓ' C =
--   -- Use a morphism from generator at earlier stage and keep new ob fixed
--   (Σ[ new-ob ∈ C fzero .ob ]
--     gens (suc (suc n))
--       (ℓ ∘f fsuc) (ℓ' ∘f fsuc)
