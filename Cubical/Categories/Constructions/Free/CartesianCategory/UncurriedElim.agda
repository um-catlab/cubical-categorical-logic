...
open import Cubical.Categories.Constructions.Free.CartesianCategory.Base hiding (elim)
...
module _ (Q : ×Quiver ℓQ ℓQ') where
  ...
  module _
    ...
    module _ (ı-ob : ∀ o → Cᴰ.ob[ ↑ o ]) where
      ...
      module _ (ı-hom : ∀ e → Cᴰ [ ↑ₑ e ][ elimOb (Q.Dom e) , elimOb (Q.Cod e)]) where
        elimHom : ∀ {A A'} (f : |FreeCartesianCategory| Q [ A , A' ]) →
          Cᴰ [ f ][ elimOb A , elimOb A' ]
        elimHom (↑ₑ t) = ı-hom t
        elimHom idₑ = Cᴰ.idᴰ
