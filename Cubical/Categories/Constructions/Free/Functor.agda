...
-- TODO: move these lemmas to Cubical.Foundations.Id
funExtRefl : ∀ {A : Type ℓ}{B : A → Type ℓ'} {f} →
             Path (Id f f) (funExtId {A = A}{B = B}{f = f} (λ x → reflId)) reflId
funExtRefl {A = A}{B = B}{f = f} =
  congPath pathToId lem ∙ pathToIdRefl
  where
   lem : Path (Path ((x : A) → B x) f f)
              (λ i x → idToPath {x = f x} reflId i)
              (λ _ → f)
   lem i j x = idToPathRefl {x = f x} i j

substIdToPath : ∀ {A : Type ℓ}{B : A → Type ℓ'} {x y : A} (p : Id x y)
                  → subst B (idToPath p) ≡ transportId B p
substIdToPath {A = A}{B}{x = x} p = funExt λ bx → J (λ by q → subst B (idToPath q) bx ≡ transportId B q bx) (lem bx) p
  where
    lem : ∀ (bx : B x) → subst B (idToPath reflId) bx ≡ transportId B reflId bx
