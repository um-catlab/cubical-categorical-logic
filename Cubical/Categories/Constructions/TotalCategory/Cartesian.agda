...
module _
  ...
  ∫C .fst = TC.∫C (Cᴰ .fst)
  ∫C .snd .fst = (C.𝟙 , Cᴰ.𝟙ᴰ) , (λ _ → (C.!t , Cᴰ.!tᴰ' _) , λ _ → ΣPathP (C.𝟙η' , Cᴰ.𝟙η'ᴰ _ _))
  ...
  ∫C .snd .snd cᴰ c'ᴰ .univProp f g = uniqueExists
    ((f .fst C.,p g .fst) , (f .snd Cᴰ.,pᴰ g .snd))
    ...
    λ fᴰ p → ΣPathP (C.×η'' (sym (cong fst (p .fst))) (symP (cong fst (p .snd))) , Cᴰ.×η''ᴰ (symP (cong snd (p .fst))) (symP (cong snd (p .snd))))
