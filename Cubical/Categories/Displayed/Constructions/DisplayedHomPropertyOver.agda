...
module Cubical.Categories.Displayed.Constructions.DisplayedHomPropertyOver where
...
private
  variable
    ℓC ℓC' ℓCᴰ ℓCᴰ' ℓH : Level

record DisplayedHomPropertyOver
  {C : Category ℓC ℓC'} (Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ') ℓH :
    Type (ℓ-max (ℓ-max (ℓ-max ℓC ℓC') (ℓ-max ℓCᴰ ℓCᴰ')) (ℓ-suc ℓH)) where
  open Fibers Cᴰ
  field
    Homᴰ : ∀ {x} {y} {f} {xᴰ : ob[ x ]} {yᴰ : ob[ y ]} →
      Hom[ f ][ xᴰ , yᴰ ] →
      Type ℓH
    isPropHomᴰᴰ : ∀ {x} {y} {f} {xᴰ : ob[ x ]} {yᴰ : ob[ y ]} →
      (fᴰ : Hom[ f ][ xᴰ , yᴰ ]) → isProp (Homᴰ fᴰ)
    idᴰ : ∀ {x}{xᴰ : ob[ x ]} → Homᴰ {y = x} (idᴰ {p = xᴰ})
    _⋆ᴰ_ : ∀ {x y z} {f g} {xᴰ : ob[ x ]} {yᴰ : ob[ y ]} {zᴰ : ob[ z ]} →
      (fᴰ : Hom[ f ][ xᴰ , yᴰ ]) → (gᴰ : Hom[ g ][ yᴰ , zᴰ ]) →
      Homᴰ fᴰ → Homᴰ gᴰ → Homᴰ (fᴰ ⋆ᴰ gᴰ)

module _ {C : Category ℓC ℓC'} {Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ'}
  (Pᴰ : DisplayedHomPropertyOver Cᴰ ℓH) where
  open Fibers Cᴰ
  open DisplayedHomPropertyOver Pᴰ
  DisplayedHomPropertyOver→Catᴰ : Categoryᴰ C ℓ-zero ℓH
  DisplayedHomPropertyOver→Catᴰ = {!!}
  -- StructureOver→Catᴰ struct where
