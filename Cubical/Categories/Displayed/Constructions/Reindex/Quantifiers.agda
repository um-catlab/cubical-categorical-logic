...
--           ][ isFibDᴰ.p*Pᴰ ∀dᴰ.vert (F ⟪ -×c.π₁ ⟫)
--           , dᴰ ]
--         ∀Reindex-elt = Dᴰ.reind ∀Reindex-elt-reind $
--           isFibDᴰ.intro (Dᴰ.reind (sym $ -×Fc.×β₁) $
--             isFibF*Dᴰ.π )
--           Dᴰ.⋆ᴰ ∀dᴰ.app
--           Dᴰ.⋆ᴰ isFibDᴰ.π
...
--         ∀Reindex-intro' :
--           ∀ {Δ}{Δᴰ}{γ : C [ Δ , Γ ]} →
--           Dᴰ.Hom[ F ⟪ -×c.×aF ⟪ γ ⟫ ⟫ ][ isFibDᴰ.p*Pᴰ Δᴰ F⟨-×c⟩.π₁ , dᴰ ]
--           → Dᴰ [ -×Fc.×aF ⟪ F ⟪ γ ⟫ ⟫ ][ isFibDᴰ.p*Pᴰ Δᴰ -×Fc.π₁
--                                       , isFibDᴰ.p*Pᴰ dᴰ (-×Fc.π₁ F⟨Γ×c⟩.,p -×Fc.π₂) ]
--         ∀Reindex-intro' γᴰ = isFibDᴰ.intro $
--           Dᴰ.reind ∀Reindex-intro-reind $
--             (isFibDᴰ.intro $ Dᴰ.reind
--               (sym $ F⟨-×c⟩.×β₁) $
--               isFibDᴰ.π)
--             Dᴰ.⋆ᴰ γᴰ
--         ∀Reindex-intro :
...
--           isFibDᴰ.intro (Dᴰ.reind (sym $ -×Fc.×β₁) $ isFibDᴰ.π)
...
--             (γᴰ : Dᴰ.Hom[ F ⟪ -×c.×aF ⟪ γ ⟫ ⟫ ][ isFibDᴰ.p*Pᴰ Δᴰ F⟨-×c⟩.π₁ , dᴰ ])
--             → ∀Reindex-intro⁻ (∀Reindex-intro γᴰ) ≡ γᴰ
--           ∀Reindex-β {Δ}{Δᴰ}{γ} γᴰ = ∀Reindex-intro⁻≡ _ ∙ (Dᴰ.rectify $ Dᴰ.≡out $
--             sym (Dᴰ.reind-filler _ _)
--             ∙ Dᴰ.⟨ refl ⟩⋆⟨ Dᴰ.⟨ ∀dᴰ.∀β {fᴰ = ∀Reindex-intro' γᴰ} ⟩⋆⟨ refl ⟩ ∙ isFibDᴰ.β ∙ (sym $ Dᴰ.reind-filler _ _) ⟩
--             ∙ sym (Dᴰ.⋆Assoc _ _ _)
--             ∙ {!!})
