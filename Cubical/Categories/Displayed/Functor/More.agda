...
module FunctorᴰNotation {C : Category ℓC ℓC'} {D : Category ℓD ℓD'} {F : Functor C D}
  {Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ'} {Dᴰ : Categoryᴰ D ℓDᴰ ℓDᴰ'}
  (Fᴰ : Functorᴰ F Cᴰ Dᴰ)
  where
  ...
  ⟨_⟩Fᴰ :
    ...
    {γᴰ : Cᴰ [ γ ][ Γᴰ , Δᴰ ]} →
    {γᴰ' : Cᴰ [ γ' ][ Γᴰ , Δᴰ ]} →
    Path Cᴰ.Hom[ _ , _ ] (γ , γᴰ) (γ' , γᴰ') →
    Path Dᴰ.Hom[ _ , _ ]
      (_ , Fᴰ .F-homᴰ γᴰ)
      (_ , Fᴰ .F-homᴰ γᴰ')
  ⟨ γᴰ≡ ⟩Fᴰ i .fst = F ⟪ γᴰ≡ i .fst ⟫
  ⟨ γᴰ≡ ⟩Fᴰ i .snd = Fᴰ .F-homᴰ (γᴰ≡ i .snd)

  ∫F-seqᴰ :
    ∀ {Γ Δ Ε : C.ob}
    {Γᴰ : Cᴰ.ob[ Γ ]} {Δᴰ : Cᴰ.ob[ Δ ]} {Εᴰ : Cᴰ.ob[ Ε ]}
    {γ : C [ Γ , Δ ]}{δ : C [ Δ , Ε ]}
    (γᴰ : Cᴰ [ γ ][ Γᴰ , Δᴰ ]) →
    (δᴰ : Cᴰ [ δ ][ Δᴰ , Εᴰ ]) →
    Path Dᴰ.Hom[ _ , _ ]
      (_ , Fᴰ .F-homᴰ (γᴰ Cᴰ.⋆ᴰ δᴰ))
      (_ , (Fᴰ .F-homᴰ γᴰ) Dᴰ.⋆ᴰ (Fᴰ .F-homᴰ δᴰ))
  ∫F-seqᴰ γᴰ δᴰ i .fst = F .Functor.F-seq _ _ i
  ∫F-seqᴰ γᴰ δᴰ i .snd = Fᴰ .F-seqᴰ γᴰ δᴰ i
