...
module _ {C : Category ℓC ℓC'} (F : Functor C C) where
  ...
  StructureOver.Hom[_][_,_] AlgStructureOver f α β =
    α ⋆⟨ C ⟩ f ≡ F-hom f ⋆⟨ C ⟩ β
  StructureOver.idᴰ AlgStructureOver = ⋆IdR _ ∙ sym (cong (_⋆ _) F-id ∙ ⋆IdL _)
  StructureOver._⋆ᴰ_ AlgStructureOver
    {f = f} {g = g} {xᴰ = α} {yᴰ = β} {zᴰ = γ} pf pg =
    sym (⋆Assoc α f g)
    ∙ cong (_⋆ g) pf
    ∙ ⋆Assoc (F-hom f) β g
    ∙ cong (F-hom f ⋆_) pg
  ...
  ALGᴰ : Categoryᴰ C ℓC' ℓC'
  ALGᴰ = StructureOver→Catᴰ AlgStructureOver

  ALG : Category _ _
  ALG = ∫C ALGᴰ

  -- An F-algebra: a carrier together with a structure map F A → A.
  Algebra : Type (ℓ-max ℓC ℓC')
  Algebra = Category.ob ALG

  InitialAlgebra : Type (ℓ-max ℓC ℓC')
  InitialAlgebra = Initial' ALG
...
module InitialAlgebraNotation {C : Category ℓC ℓC'} {F : Functor C C}
  ...
  fold : ∀ (B : Category.ob (ALG F)) → ALG F [ μF , B ]
  fold B = absurd

  fold-unique : ∀ {B : Category.ob (ALG F)}
    (f g : ALG F [ μF , B ]) → f ≡ g
