...
module _ {C : Category ℓC ℓC'} (Mon : Monad C) where
  ...
  EMStructureOver : StructureOver (∫C (AlgCatᴰ (fst Mon))) ℓC' ℓ-zero
  ...
  EMCatᴰ : Categoryᴰ (∫C (AlgCatᴰ (fst Mon))) ℓC' ℓ-zero
  EMCatᴰ = StructureOver→Catᴰ EMStructureOver

  -- The Eilenberg–Moore category.
  EMCategory : Category (ℓ-max ℓC ℓC') (ℓ-max ℓC' ℓC')
  EMCategory = ∫C EMCatᴰ

-- A morphism of algebras is determined by the carrier map
module _ {C : Category ℓC ℓC'} {Mon : Monad C} where

  emHom≡ : {X Y : Category.ob (EMCategory Mon)}
    {f g : EMCategory Mon [ X , Y ]}
    → f .fst .fst ≡ g .fst .fst → f ≡ g
  emHom≡ p = ΣPathP
    (Σ≡Prop (λ _ → hasPropHomsStructureOver (AlgStructureOver (fst Mon)) _ _ _) p
    , refl)
