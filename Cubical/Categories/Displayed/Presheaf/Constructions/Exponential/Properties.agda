  ...
  module _ {P : Presheaf C ℓP}
    ...
    module _ (Rᴰ : Presheafᴰ P Cᴰ ℓRᴰ) where
      ...
      ⇒PshSmall-introⱽ :
        ...
        → PshHomⱽ Rᴰ ((Pᴰ , _×ⱽ_*Pᴰ) ⇒PshSmallⱽ Qᴰ)
      ⇒PshSmall-introⱽ α⟨pᴰ⟩ .N-obᴰ rᴰ =
        α⟨pᴰ⟩ .N-obᴰ ((π₁LR _ _ Rᴰ.⋆ⱽᴰ rᴰ) , Pᴰ.reind (P.⋆IdL _) (π₂LR _ _))
      ⇒PshSmall-introⱽ α⟨pᴰ⟩ .N-homᴰ = Qᴰ.rectify $ Qᴰ.≡out $
        α⟨pᴰ⟩.N-obᴰ⟨
          ΣPathPSnd ((sym $ Rᴰ.reind-filler _ _) ∙ (sym $ Rᴰ.⋆Assoc _ _ _)) -- refl or Assoc
                    (Pᴰ.rectify $ Pᴰ.≡out $ (sym $ Pᴰ.reind-filler _ _) ∙ {!!} ) ⟩
        ∙ α⟨pᴰ⟩.∫PshHom .N-hom _ _ _ _
        ∙ {!!}
        ∙ {!!}
        where
          module α⟨pᴰ⟩ = PshHomᴰ α⟨pᴰ⟩
