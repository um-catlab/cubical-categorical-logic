...
module _
  ...
  module _
    ...
    module Fᴰ-weakening (Fᴰ : Functorᴰ F Cᴰ Dᴰ) where
      ...
      -- The component of Fᴰ-weakening-NatTransᴰ, in reind normal form: since
      -- introⁿ (mkⁿ (sym e) v) is definitionally introπF* (Dᴰ.reind e v),
      -- this is the same component, presented so the normal-form lemmas apply.
      weakening-obᴰⁿ : ∀ {Γ}(Γᴰ : Cᴰ.ob[ Γ ])
        → ∀ⱽDᴰ.Lⁿ (Fᴰ .F-obᴰ Γᴰ) (Fᴰ .F-obᴰ (∀ⱽCᴰ.weakenπFᴰ .F-obᴰ Γᴰ)) mapProdStr
      weakening-obᴰⁿ Γᴰ = ∀ⱽDᴰ.mkⁿ
        (sym $ cong (F .F-hom) (C.⋆IdL _) ∙ (sym -×Fc.×β₁))
        (Fᴰ .F-homᴰ (elementⱽ (π₁*C Γᴰ)))

      Fᴰ-weakening-NatTransᴰ :
        NatTransᴰ prodStrNatTrans
          (Fᴰ ∘Fᴰ ∀ⱽCᴰ.weakenπFᴰ)
          (∀ⱽDᴰ.weakenπFᴰ ∘Fᴰ Fᴰ)
      Fᴰ-weakening-NatTransᴰ .NatTransᴰ.N-obᴰ xᴰ = ∀ⱽDᴰ.introⁿ (weakening-obᴰⁿ xᴰ)
      Fᴰ-weakening-NatTransᴰ .NatTransᴰ.N-homᴰ {xᴰ = xᴰ} {yᴰ = yᴰ} fᴰ =
        Dᴰ.rectifyOut $
          ∀ⱽDᴰ.introⁿ-natural
            (Fᴰ .F-homᴰ (∀ⱽCᴰ.weakenπFᴰ .F-homᴰ fᴰ)) (weakening-obᴰⁿ yᴰ)
          ∙ ∀ⱽDᴰ.introⁿ≡
              (∀ⱽDᴰ._⋆Lⁿ_ (Fᴰ .F-homᴰ (∀ⱽCᴰ.weakenπFᴰ .F-homᴰ fᴰ))
                          (weakening-obᴰⁿ yᴰ))
              {f = ∀ⱽDᴰ.introⁿ (weakening-obᴰⁿ xᴰ)
                   Dᴰ.⋆ᴰ ∀ⱽDᴰ.weakenπFᴰ .F-homᴰ (Fᴰ .F-homᴰ fᴰ)}
              (-×Fc.,p-extensionality
                (D.⋆Assoc _ _ _
                ∙ D.⟨ refl ⟩⋆⟨ -×Fc.×β₁ ⟩
                ∙ (sym $ F .F-seq _ _)
                ...
                ∙ (sym $ D.⋆Assoc _ _ _)))
              ((sym $ ∫.∫F Fᴰ .F-seq _ _)
              ∙ cong (∫.∫F Fᴰ .F-hom) (∀ⱽCᴰ.β-weakenⁿ fᴰ)
              ∙ ∫.∫F Fᴰ .F-seq _ _
              ∙ Dᴰ.⟨ sym $ ∀ⱽDᴰ.βⁿ (weakening-obᴰⁿ xᴰ) ⟩⋆⟨ refl ⟩
              ∙ Dᴰ.⋆Assoc _ _ _
              ∙ Dᴰ.⟨ refl ⟩⋆⟨ sym $ ∀ⱽDᴰ.βⁿ (∀ⱽDᴰ.weakenⁿ (Fᴰ .F-homᴰ fᴰ)) ⟩
              ∙ (sym $ Dᴰ.⋆Assoc _ _ _))
