...
module _ {C : Category ℓC ℓC'}{Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ'} where
  ...
  module _ {P : Presheaf C ℓP}{Q : Presheaf C ℓQ} (α : PshHom P Q) (Qᴰ : Presheafᴰ Q Cᴰ ℓQᴰ) where
    private
      module α*Qᴰ = PresheafᴰNotation (reind α Qᴰ)
      module P = PresheafNotation P
      module Q = PresheafNotation Q
      module Qᴰ = PresheafᴰNotation Qᴰ
    reindPshᴰ-Path :
      ∀ {x xᴰ}
        {p : P.p[ x ]}{qᴰ : Qᴰ.p[ α .N-ob x p ][ xᴰ ]}
        {p' : P.p[ x ]}{qᴰ' : Qᴰ.p[ α .N-ob x p' ][ xᴰ ]}
      → p ≡ p'
      → Path Qᴰ.p[ _ ] (α .N-ob x p , qᴰ) (α .N-ob x p' , qᴰ')
      → Path α*Qᴰ.p[ _ ] (p , qᴰ) (p' , qᴰ')
    reindPshᴰ-Path = change-base (α .N-ob _) Q.isSetPsh
    ...
    reindPshᴰ-Path⁻ :
      ...
      → Path α*Qᴰ.p[ _ ] (p , qᴰ) (p' , qᴰ')
      → Path Qᴰ.p[ _ ] (α .N-ob x p , qᴰ) (α .N-ob x p' , qᴰ')
    reindPshᴰ-Path⁻ = change-base⁻ (α .N-ob _)

  module _ {P : Presheaf C ℓP}{Q : Presheaf C ℓQ} {α : PshHom P Q} {Qᴰ : Presheafᴰ Q Cᴰ ℓQᴰ}
    where
    private
      module Qᴰ = PresheafᴰNotation Qᴰ
    reind-π : PshHomᴰ α (reind α Qᴰ) Qᴰ
    reind-π .N-obᴰ = λ z → z
    reind-π .N-homᴰ = sym $ Qᴰ.reind-filler _ _
  ...
  module _ {P : Presheaf C ℓP}{Q : Presheaf C ℓQ}{R : Presheaf C ℓR}
    ...
    reind-introᴰ αβᴰ .N-homᴰ =
      reindPshᴰ-Path β Rᴰ (α .N-hom _ _ _ _)
        (αβᴰ .N-homᴰ ∙ Rᴰ.reind-filler _ _)

    opaque
      reind-β :
        (αβᴰ : PshHomᴰ (α ⋆PshHom β) Pᴰ Rᴰ) →
        Path (PshCᴰ.Hom[ _ , _ ]) (_ , reind-introᴰ αβᴰ ⋆PshHomᴰ reind-π) (_ , αβᴰ)
      reind-β αβᴰ = makePshHomᴰPath refl λ pᴰ → refl

      reind-βᴰ :
        (αβᴰ : PshHomᴰ (α ⋆PshHom β) Pᴰ Rᴰ) →
        reind-introᴰ αβᴰ ⋆PshHomᴰ reind-π ≡ αβᴰ
      reind-βᴰ αβᴰ = PshCᴰ.rectify $ PshCᴰ.≡out $ reind-β αβᴰ

      reind-η :
        (αᴰ : PshHomᴰ α Pᴰ (reind β Rᴰ)) →
        Path PshCᴰ.Hom[ _ , _ ] (_ , αᴰ) (_ , reind-introᴰ (αᴰ ⋆PshHomᴰ reind-π))
      reind-η αᴰ = makePshHomᴰPath refl λ pᴰ → refl

      reind-ηᴰ :
        (αᴰ : PshHomᴰ α Pᴰ (reind β Rᴰ)) →
        reind-introᴰ (αᴰ ⋆PshHomᴰ reind-π) ≡ αᴰ
      reind-ηᴰ αᴰ = PshCᴰ.rectify $ PshCᴰ.≡out $ makePshHomᴰPath refl λ pᴰ → refl
  ...
  module _ {Q : Presheaf C ℓQ}{R : Presheaf C ℓR}
    (β : PshHom Q R)
    (Rᴰ : Presheafᴰ R Cᴰ ℓRᴰ)
    where
    private
    ...
    reind-UMP :
      ...
      → IsoOver' {A = PshHom P Q}{B = PshHom P Q}
          idIso
          (λ α → PshHomᴰ (α PshC.⋆ β) Pᴰ Rᴰ)
          (λ α → PshHomᴰ α Pᴰ (reind β Rᴰ))
    reind-UMP = PointwiseIso→IsoOver'Id _ _ (λ _ → reind-UMP')
...
--     reindPathIsoⱽ : {!PshCᴰ.ISOCᴰ!} -- PshCᴰ.ISOCᴰ (reind α Qᴰ) (reind β Qᴰ)
--     reindPathIsoⱽ = {!!}
