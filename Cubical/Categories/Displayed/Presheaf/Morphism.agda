...
open import Cubical.Categories.LocallySmall
  using (LocallySmallCategory; LocallySmallCategoryᴰ; LEVEL; _⊘_; Liftω; Σω; _,_; liftω; mapω')
...
module _ {C : Category ℓC ℓC'} where
  private
    module C = Category C
    PshC = PRESHEAF C
    module PshC = LocallySmallCategoryᴰ PshC
...
-- -- Next we relate our notion to Natural Transformations
...
--   -- TODO make this a no-eta-equality record
--   PshIsoᴰ : Type _
--   PshIsoᴰ =
...
--         PshHomEqPshHomᴰ .N-obᴰ = PshHomEqPshHomᴰ-N-obᴰ (_ , eq-N-ob)
--         PshHomEqPshHomᴰ .N-homᴰ = Qᴰ.rectify (PshHomEqPshHomᴰ-N-homᴰ (_ , eq-N-ob))
--       module _ (α≡β : α ≡ β) where
--         -- Change the base PshHom of a PshHomᴰ along
--         -- a path of PshHoms

--         -- This argument order is inconsistent with reind
...
-- --   private
-- --     PshCᴰ = PRESHEAFᴰ Cᴰ
-- --     module PshCᴰ = LocallySmallCategoryᴰ PshCᴰ
-- --   module _ {P : Presheaf C ℓP}{Q : Presheaf C ℓQ}
-- --     {α : PshIso P Q} {Pᴰ : Presheafᴰ P Cᴰ ℓPᴰ}{Qᴰ : Presheafᴰ Q Cᴰ ℓQᴰ} where
-- --     PshIsoᴰ→PshCatIsoᴰ : PshIsoᴰ α Pᴰ Qᴰ → LocallySmall.CatIsoᴰ PshCᴰ {!!} {!!} {!!}
-- --     PshIsoᴰ→PshCatIsoᴰ = {!!}
