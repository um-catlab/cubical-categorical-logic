...
module _ {C : Category ℓC ℓC'} (P : LRPresheaf C ℓP) (Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ') (ℓPᴰ : Level) where
  ...
  module LRᴰPresheafᴰNotation (Pᴰ : LRᴰPresheafᴰ) where
    ...
    opaque
      ...
      LRᴰpairᴰ : {Γ : C.ob} {Γᴰ : Cᴰ.ob[ Γ ]} {Δ : C.ob} {Δᴰ : Cᴰ.ob[ Δ ]}
                 {(f , p) : (C [ Δ , Γ ]) × P.p[ Δ ]} →
                 (Cᴰ [ f ][ Δᴰ , Γᴰ ]) × p[ p ][ Δᴰ ] →
                 Cᴰ [ ueᴰ.ue.intro {Γᴰ = Γᴰ} (f , p) ][ Δᴰ , Γᴰ ×ᴰPᴰ ]
      LRᴰpairᴰ = ueᴰ.introᴰ
...
module ExponentialᴰNotation {C : Category ℓC ℓC'} {Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ'}
  ...
  opaque
    ...
    --     (γᴰ : Cᴰ [ γ ][ Δᴰ , Γᴰ ])
    --     (fᴰ : Cᴰ [ f ][ Γᴰ -×ᴰAᴰ.×ᴰPᴰ , Bᴰ ])
    --     → (γᴰ impr⋆ᴰ fᴰ) Cᴰ.∫≡ {!(γᴰ Aᴰ⇒BᴰPshᴰ.⋆ᴰ ?)!}
    --     -- (γᴰ Aᴰ⇒BᴰPshᴰ.⋆ᴰ fᴰ)
    ...
    ⇒βᴰ : ∀ {Γ}{Γᴰ}{f : C [ (Γ ×A) .vertex , B ]}
        → (fᴰ : Cᴰ [ f ][  Γᴰ -×ᴰAᴰ.×ᴰPᴰ , Bᴰ ])
        → (-×ᴰAᴰ.LRᴰpairᴰ ((-×ᴰAᴰ.π₁ᴰ Cᴰ.⋆ᴰ λᴰ fᴰ) , -×ᴰAᴰ.π₂ᴰ) Cᴰ.⋆ᴰ appᴰ) Cᴰ.∫≡ fᴰ
    ⇒βᴰ fᴰ = Cᴰ.reind-filler _ _ ∙ (Cᴰ.≡in $ ueᴰ.βᴰ fᴰ)

    ⇒ηᴰ-Ty : ∀ {Γ}{Γᴰ}{f : C [ Γ , A⇒B.⇒ue.vertex ]}
        → (fᴰ : Cᴰ [ f ][  Γᴰ , Aᴰ⇒ᴰBᴰ .fst ])
        → Type (ℓ-max ℓC' ℓCᴰ')
    ⇒ηᴰ-Ty fᴰ = fᴰ Cᴰ.∫≡ λᴰ (-×ᴰAᴰ.×ᴰPᴰ-Fᴰ .F-homᴰ fᴰ Cᴰ.⋆ᴰ appᴰ)

    ⇒ηᴰ : ∀ {Γ}{Γᴰ}{f : C [ Γ , A⇒B.⇒ue.vertex ]}
        → (fᴰ : Cᴰ [ f ][  Γᴰ , Aᴰ⇒ᴰBᴰ .fst ])
        → ⇒ηᴰ-Ty fᴰ
    ⇒ηᴰ fᴰ = (Cᴰ.≡in $ ueᴰ.ηᴰ fᴰ) ∙ ueᴰ.cong-introᴰ (sym $ Cᴰ.reind-filler _ _)
