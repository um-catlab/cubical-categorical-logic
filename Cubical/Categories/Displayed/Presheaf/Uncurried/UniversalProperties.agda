...
module _ {C : Category ℓC ℓC'} (Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ') where
  ...
  Terminalⱽ x = Representableⱽ Cᴰ x (UnitPshᴰ {P = C [-, x ]})

  Terminalsⱽ : Type _
  Terminalsⱽ = ∀ x → Terminalⱽ x

  TerminalᴰSpec : Presheafᴰ UnitPsh Cᴰ ℓ-zero
  TerminalᴰSpec = UnitPshᴰ {P = UnitPsh}

  Terminalᴰ : ∀ (term : Terminal' C) → Type _
  Terminalᴰ = UniversalElementᴰ Cᴰ UnitPsh TerminalᴰSpec
  ...
  BinProductⱽSpec {x} xᴰ yᴰ = _×ⱽPsh_ {P = C [-, x ]} (Cᴰ [-][-, xᴰ ]) (Cᴰ [-][-, yᴰ ])

  BinProductⱽ : ∀ {x} → (xᴰ yᴰ : Cᴰ.ob[ x ]) → Type _
  BinProductⱽ {x} xᴰ yᴰ = Representableⱽ Cᴰ x (BinProductⱽSpec xᴰ yᴰ)
  ...
  BinProductᴰSpec {A}{B} A×B Aᴰ Bᴰ =
    _×ᴰPsh_ {P = C [-, A ]}{Q = C [-, B ]} (Cᴰ [-][-, Aᴰ ]) (Cᴰ [-][-, Bᴰ ])

  BinProductᴰ : ∀ {A B} → (A×B : BinProduct C (A , B)) (Aᴰ : Cᴰ.ob[ A ]) (Bᴰ : Cᴰ.ob[ B ]) → Type _
  BinProductᴰ {A}{B} A×B Aᴰ Bᴰ =
    UniversalElementᴰ Cᴰ _ (BinProductᴰSpec A×B Aᴰ Bᴰ) A×B
  ...
  module BinProductᴰNotation {A B Aᴰ Bᴰ} (A×B : BinProduct C (A , B)) (Aᴰ×ᴰBᴰ : BinProductᴰ A×B Aᴰ Bᴰ) where
    ...
    open UniversalElementᴰNotation Cᴰ (BinProductProf C ⟅ A , B ⟆) (BinProductᴰSpec A×B Aᴰ Bᴰ) {ue = A×B} Aᴰ×ᴰBᴰ
    ...
    ×ηᴰ {Γ} {Γᴰ} {f} fᴰ = Cᴰ.rectify $ Cᴰ.≡out $
      Cᴰ.≡in (ηᴰ {f = f} fᴰ)
      ∙ cong (∫PshIsoᴰ {α = A×B.asPshIso} (asReprᴰ .snd) .nIso (Γ , Γᴰ) .fst)
          (ΣPathPᴰ
