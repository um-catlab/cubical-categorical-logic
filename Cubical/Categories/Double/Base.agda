...
record DoubleCategory ℓC ℓH ℓV ℓS : {!!} where
  no-eta-equality
  field
    ob : Type ℓC
  ...
  -- ⟨_⟩⋆ⱽ⟨_⟩ : ∀ {x y z} {f f' : Hom₁[ x , y ]} {g g' : Hom₁[ y , z ]} →
  --   f ≡ f' → g ≡ g' → f ⋆₁ g ≡ f' ⋆₁ g'
  -- ⟨ f≡ ⟩⋆ⱽ⟨ g≡ ⟩ = cong₂ _⋆₁_ f≡ g≡
...
module _ {ℓC ℓC' ℓD ℓD' ℓE ℓE' ℓS ℓR}
  ...
  seqRelator :
    Relatoro* C ℓS D  →
    Relatoro* D ℓR E  →
    Relatoro* C (ℓ-max (ℓ-max (ℓ-max ℓD ℓD') ℓS) ℓR) E
...
module _ {ℓC ℓC' ℓS} where
  CAT : DoubleCategory _ _ _ _
  CAT .ob = Category ℓC ℓC'
  CAT .Homⱽ[_,_] = Functor
  CAT .idⱽ = Id
  CAT ._⋆ⱽ_ G F = F ∘F G
  CAT .⋆ⱽIdL _ = F-lUnit
  CAT .⋆ⱽIdR _ = F-rUnit
  CAT .⋆ⱽAssoc _ _ _ = F-assoc
  CAT .Homᴴ[_,_] C D = Relatoro* C (ℓ-max (ℓ-max ℓC ℓC') ℓS) D
  CAT .idᴴ {x = C} = compF (LiftF {ℓ = ℓC'} {ℓ' = ℓ-max ℓC ℓS}) (HomBif C)
  CAT ._⋆ᴴ_ S R = seqRelator S R
  -- TODO show these are iso then use univalence
  CAT .⋆ᴴIdL S = {!!}
  CAT .⋆ᴴIdR S = {!!}
  CAT .⋆ᴴAssoc = {!!}
  -- CAT .isSetHomᴴ = {!!}
  CAT .Sq S R F G = RelatorHom S (R ∘Flr ((F ^opF) , G))
  CAT .idⱽSq = eqToPshHom _ Eq.refl Eq.refl
  CAT .idᴴSq {v = F} =
    mkRelatorHom (λ c d z → lift (F .Functor.F-hom (z .lower)))
      (λ _ _ _ _ _ → cong lift (F .F-seq _ _))
      (λ _ _ _ _ _ → cong lift (F .F-seq _ _))
  CAT ._⋆ⱽSq_ {↑f = ↑f} {→f = →f} {↓f' = ↓f'} {→f' = →f'} α β =
...
--   CAT .Homᴴ-ℓ x y = {!!}
--   CAT .Homᴴ[_,_] C D = Relatoro* C {!!} D
--   CAT .idᴴ {x = C} = HomBif C
--   CAT ._⋆ᴴ_ S R = seqRelator {!S!} {!!}
--   CAT .⋆ᴴIdL = {!!}
--   CAT .⋆ᴴIdR = {!!}
--   CAT .⋆ᴴAssoc = {!!}
--   CAT .isSetHomᴴ = {!!}
