...
module _ (C : Category ℓc ℓc') (D : Category ℓd ℓd') where
  ...
  record StrictFunctor : Type (ℓ-max (ℓ-max ℓc ℓc') (ℓ-max ℓd ℓd')) where
    ...
    field
      ...
      -- The fords are Eq-valued and ORIENTED FORWARDS, i.e. the same
      -- direction the displayed category's idᴰ/⋆ᴰ want them.  That is
      -- what lets _S∘_ and reindexS hand the witness over verbatim,
      -- with no `sym` anywhere -- and hence what lets the ford be Eq,
      -- whose transport computes at refl.
      F-id  : {x : C .ob} (f : C [ x , x ])
        → C .id Eq.≡ f
        → D .id Eq.≡ F-hom f
      F-seq : {x y z : C .ob}
...
compLS {C = C}{D = D}{E = E} F G = mkBifunctorSepAx B where
  ...
  B .Bif-L-id {c}{d} = (λ i → F ⟪ Eq.eqToPath (Eq.sym (G .F-id (Category.id C) Eq.refl)) i ⟫l) ∙ F .Bif-L-id
  B .Bif-L-seq {d = d} f f' =
    (λ i → F ⟪ Eq.eqToPath (Eq.sym (G .F-seq f f' _ Eq.refl)) i ⟫l) ∙ F .Bif-L-seq _ _
...
compRS {C = C}{D = D}{E = E} F H = mkBifunctorSepAx B where
  ...
  B .Bif-R-id {c}{d} = (λ i → F ⟪ Eq.eqToPath (Eq.sym (H .F-id (Category.id D) Eq.refl)) i ⟫r) ∙ F .Bif-R-id
  B .Bif-R-seq g g' =
    (λ i → F ⟪ Eq.eqToPath (Eq.sym (H .F-seq g g' _ Eq.refl)) i ⟫r) ∙ F .Bif-R-seq _ _
