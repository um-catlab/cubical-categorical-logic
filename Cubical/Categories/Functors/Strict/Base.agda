...
module _ {C : Category ℓc ℓc'} {D : Category ℓd ℓd'} {E : Category ℓe ℓe'}
  ...
  (G S∘ F) .F-seq f g h e =
    G .F-seq (F .F-hom f) (F .F-hom g) (F .F-hom h)
             (sym $ F .F-seq f g h e)
...
module _ {B : Category ℓc ℓc'} {C : Category ℓd ℓd'}
         ...
         where
  test-Assoc : (F : StrictFunctor B C) (G : StrictFunctor C D)
               (H : StrictFunctor D E)
             → ((H S∘ G) S∘ F) ≡ (H S∘ (G S∘ F))
...
compLS {C = C}{D = D}{E = E} F G = mkBifunctorSepAx B where
  ...
  B .Bif-L-id {c}{d} =
    (λ i → F ⟪ G .F-id (Category.id C) refl i ⟫l) ∙ F .Bif-L-id
  B .Bif-L-seq {d = d} f f' =
...
compRS {C = C}{D = D}{E = E} F H = mkBifunctorSepAx B where
  ...
  B .Bif-R-id {c}{d} =
    (λ i → F ⟪ H .F-id (Category.id D) refl i ⟫r) ∙ F .Bif-R-id
  B .Bif-R-seq g g' =
