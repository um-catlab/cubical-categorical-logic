...
module _ {C : Category ℓc ℓc'} {D : Category ℓd ℓd'} where
  ...
  -- The naturality ford is Eq-valued and oriented forwards, matching
  -- the convention for StrictFunctor's F-id/F-seq: the hypothesis is
  -- the *defining equation of the ford variable* `g`, so it is always
  -- discharged by `Eq.refl` at call sites and its transport reduces.
  -- The conclusion stays Path-valued: it is the genuine naturality
  -- square, which for a general D is not a definitional equation.
  ...
  Strict-N-hom-Type F G ϕ = {x y : C.ob} (f : C [ x , y ])
    ...
    → (ϕ x D.⋆ G .F-hom f) Eq.≡ g
    → F .F-hom f D.⋆ ϕ y ≡ g


  record StrictNatTrans (F G : StrictFunctor C D)
    : Type (ℓ-max (ℓ-max ℓc ℓc') ℓd') where
    constructor natTrans
    field
      N-ob : Strict-N-ob-Type F G
      N-hom :  Strict-N-hom-Type F G N-ob

  record StrictNatIso (F G : StrictFunctor C D)
  ...
  idStrictTrans F .N-hom f g p =
    D.⋆IdR _ ∙ (sym $ D.⋆IdL _) ∙ Eq.eqToPath p
  ...
  seqStrictTrans α β .N-hom f g p =
    compSq D (N-hom α f _ Eq.refl) (N-hom β f _ Eq.refl) ∙ Eq.eqToPath p

  makeStrictNatTransPath : {F G : StrictFunctor C D}
    {α β : StrictNatTrans F G} →
    α .N-ob ≡ β .N-ob → α ≡ β
  makeStrictNatTransPath p i .N-ob = p i
  makeStrictNatTransPath {F = F}{G = G}{α = α}{β = β} p i .N-hom f g q =
    help i q
    where
    help : PathP
      (λ i → p i _ D.⋆ G .F-hom f Eq.≡ g → F .F-hom f D.⋆ p i _ ≡ g)
...
module _ {C : Category ℓc ℓc'} where
  ...
  -- Iso variant
  record SPshIso (P Q : StrictPresheaf C ℓc)
    : Type (ℓ-max (ℓ-max ℓc ℓc') ℓc) where
    field
      trans : SPshHom P Q
