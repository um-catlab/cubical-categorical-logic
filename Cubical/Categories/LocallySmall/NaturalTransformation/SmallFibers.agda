    ...
    module _ (hasContrHomsC : hasContrHoms C.cat) where
      module _ (isoOpFibCᴰ : isIsoOpFibration Cᴰ) where
        module _ {c c' : C.ob} where
          LiftF = contrHomsIsoOpFibF hasContrHomsC Cᴰ (liftω c) (liftω c')
            (λ cᴰ → isoOpFibCᴰ cᴰ
              (mkContrHomsIso C.cat hasContrHomsC (liftω c) (liftω c')))
        ...
        module _ {c c' c'' : C.ob} where
          ...
          LiftF-seq .SmallNT.NatTrans.N-ob cᴰ =
            Cᴰ.reind (isContr→isProp (hasContrHomsC _ _) _ _) $
        ...
        module _ {c}{d}(F : Functor c d) where
          ...
          idTrans .N-ob cᴰ =
            F-hom F (Cᴰ.reind (isContr→isProp (hasContrHomsC _ _) _ _) $
              isoOpFibCᴰ _ _ .f*cᴰIsoᴰ .funᴰ)
          idTrans .N-hom f =
            Dᴰ.reind-filler _ _
                ...
                (λ i → ∀ x →
                  Dᴰ.Hom[ g≡g' i ][ F.F-ob (liftω x) ,
                                    G.F-ob (isoOpFibCᴰ (liftω x)
                                      (mkContrHomsIso C.cat hasContrHomsC (liftω c) (liftω c')) .f*cᴰ) ])
                (α .N-ob)
                (β .N-ob)
        ...
        module _
          ...
          makeNatTransPath :
            Path (Σ[ (_ , h) ∈ (C^op.Hom[ liftω c , liftω c' ] × D.Hom[ liftω d , liftω d' ]) ] NatTrans h F G)
              ((hasContrHomsC (liftω c') (liftω c) .fst , g) , α) (_ , β)
          makeNatTransPath =
