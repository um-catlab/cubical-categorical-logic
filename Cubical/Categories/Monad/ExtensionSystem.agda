...
module _ (C : Category ℓ ℓ') where
  ...
  Monad≡ : {M N : Monad C}
          → (funcEq : (fst M) ≡ (fst N))
          → PathP (λ i → _) (IsMonad.η (snd M)) (IsMonad.η (snd N))
          → PathP (λ i → _) (IsMonad.μ (snd M)) (IsMonad.μ (snd N))
          → M ≡ N
  fst (Monad≡ hFunc hη hμ i) = hFunc i
  snd (Monad≡ hFunc hη hμ i) .IsMonad.η = hη i
  snd (Monad≡ hFunc hη hμ i) .IsMonad.μ = hμ i
  snd (Monad≡ hFunc hη hμ i) .IsMonad.idl-μ = {!   !}
  snd (Monad≡ hFunc hη hμ i) .IsMonad.idr-μ = {!   !}
  snd (Monad≡ hFunc hη hμ i) .IsMonad.assoc-μ = {!   !}

  ExtensionSystem≡ : {M N : ExtensionSystem}
                   → (funcEq : (fst M) ≡ (fst N))
                   → PathP (λ i → _) (η (snd M)) (η (snd N))
                   → PathP (λ i → _) (bind (snd M)) (bind (snd N))
                   → M ≡ N
  fst (ExtensionSystem≡ hFunc hη hbind i) = hFunc i
  snd (ExtensionSystem≡ hFunc hη hbind i) .η = {!    !}
  snd (ExtensionSystem≡ hFunc hη hbind i) .bind = {!   !}
  snd (ExtensionSystem≡ hFunc hη hbind i) .bind-r = {!   !}
  snd (ExtensionSystem≡ hFunc hη hbind i) .bind-l = {!   !}
  snd (ExtensionSystem≡ hFunc hη hbind i) .bind-comp = {!   !}

  ExtensionSystem≅Monad : Iso (ExtensionSystem) (Monad C)
  ExtensionSystem≅Monad .fun = ExtensionSystem→Monad
  ExtensionSystem≅Monad .inv = Monad→ExtensionSystem
  ExtensionSystem≅Monad .rightInv M =
    Monad≡
    (Functor≡
    (λ c → refl)
    (λ f → {! (cong
      (λ e → (IsMonad.μ (M .snd) ⟦ c' ⟧) ∘⟨ C ⟩ e)
      (F-seq (M .fst) (IsMonad.η (M .snd) ⟦ c' ⟧) f)
    )
    ∙ ((C .⋆Assoc)
      ((M .fst) ⟪ f ⟫)
      ((M .fst) ⟪ (IsMonad.η (M .snd) ⟦ c' ⟧) ⟫)
      (IsMonad.μ (M .snd) ⟦ c' ⟧)
    ) 
    ∙ ((M .snd .IsMonad.idr-μ)) 
    ∙ ((C .⋆IdR) ((M .fst) ⟪ f ⟫)) !}))
    refl
    refl
  ExtensionSystem≅Monad .leftInv M =
    ExtensionSystem≡
    refl
    refl
    refl
