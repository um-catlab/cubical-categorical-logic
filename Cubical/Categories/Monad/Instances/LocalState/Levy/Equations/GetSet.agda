...
get-set-currentᵗ : ∀ {Γ A}
  (i : Γ ⊢ Ref) (t : Γ ⊢ T ⟅ A ⟆) →
  getᵗ {A = A} i (setᵗ {A = A} (wkᵗ i) varᵗ (wkᵗ t)) ≡ t
get-set-currentᵗ {Γ = Γ} {A = A} i t =
  makePshHomStrictPath (funExt λ n → funExt λ γ →
    funExt₃ λ m n≤m σ →
      ...
      in
      getᵗ-β {Γ = Γ} {A = A} i
        (setᵗ {A = A} (wkᵗ i) varᵗ (wkᵗ t)) n γ m n≤m σ
...
set-get-sameᵗ : ∀ {Γ A}
  (i : Γ ⊢ Ref) (b : Γ ⊢ VVal)
  (k : Γ CC.× VVal ⊢ T ⟅ A ⟆) →
  setᵗ {A = A} i b (getᵗ {A = A} i k) ≡ setᵗ {A = A} i b (k [ b ]ᵗ)
...
set-set-sameᵗ : ∀ {Γ A}
  ...
  setᵗ {A = A} i b (setᵗ {A = A} i c t) ≡ setᵗ {A = A} i c t
  ...
  makePshHomStrictPath (funExt λ n → funExt λ γ →
    funExt₃ λ m n≤m σ →
      let
        ...
        vb = b .N-ob n γ
      in
      setᵗ-β {Γ = Γ} {A = A} i b (setᵗ {A = A} i c t) n γ m n≤m σ
...
get-get-commuteᵗ : ∀ {Γ A}
  ...
  getᵗ {A = A} i (getᵗ {A = A} (wkᵗ j) k) ≡
  getᵗ {A = A} j (getᵗ {A = A} (wkᵗ i) (exchangeᵗ k))
get-get-commuteᵗ {Γ = Γ} {A = A} i j k =
  makePshHomStrictPath (funExt λ n → funExt λ γ →
    funExt₃ λ m n≤m σ →
      ...
      in
      getᵗ-β {Γ = Γ} {A = A} i (getᵗ {A = A} (wkᵗ j) k) n γ m n≤m σ
      ...
      ∙ sym (getᵗ-β {Γ = Γ} {A = A} j
          (getᵗ {A = A} (wkᵗ i) (exchangeᵗ k)) n γ m n≤m σ))
...
set-set-commuteᵗ : ∀ {Γ A}
  ...
  (b c : Γ ⊢ VVal) (t : Γ ⊢ T ⟅ A ⟆) →
  setᵗ {A = A} i b (setᵗ {A = A} j c t) ≡ setᵗ {A = A} j c (setᵗ {A = A} i b t)
  ...
  makePshHomStrictPath (funExt λ n → funExt λ γ →
    funExt₃ λ m n≤m σ →
      let
        ...
        vc = c .N-ob n γ
      in
      setᵗ-β {Γ = Γ} {A = A} i b (setᵗ {A = A} j c t) n γ m n≤m σ
      ...
      ∙ sym (setᵗ-β {Γ = Γ} {A = A} j c (setᵗ {A = A} i b t)
          n γ m n≤m σ))
...
set-get-commuteᵗ : ∀ {Γ A}
  ...
  setᵗ {A = A} i b (getᵗ {A = A} j k) ≡
  getᵗ {A = A} j (setᵗ {A = A} (wkᵗ i) (wkᵗ b) k)
set-get-commuteᵗ {Γ = Γ} {A = A} i j i≢j b k =
  makePshHomStrictPath (funExt λ n → funExt λ γ →
    funExt₃ λ m n≤m σ →
      let
        ...
        store-right≡left =
          cong₂ (λ r v → updateStore {n = m} r v σ)
            (funExt⁻ (Ref .F-id {x = m}) (i .N-ob m γₘ)
              ∙ sym (i .N-hom m n n≤m γ _ refl))
            (sym (b .N-hom m n n≤m γ _ refl))
      in
      setᵗ-β {Γ = Γ} {A = A} i b (getᵗ {A = A} j k) n γ m n≤m σ
      ...
      ∙ sym (getᵗ-β {Γ = Γ} {A = A} j
          (setᵗ {A = A} (wkᵗ i) (wkᵗ b) k) n γ m n≤m σ))
