...
module Cubical.Categories.Monoidal.Enriched.Presheaf where
        ...
        self .⋆Assoc P Q R S = 
            makeNatTransPath (funExt λ c → funExt λ{ (f , g , h) → 
              makePshHomPath (funExt λ c' → funExt λ{ (j , Pc') → 
                ...
                ∙ cong (λ e → h .PshHom.N-ob c' ((𝓒 ⋆ id 𝓒) ((𝓒 ⋆ id 𝓒) j), g .PshHom.N-ob c' ((𝓒 ⋆ id 𝓒) ((𝓒 ⋆ id 𝓒) j) , f .PshHom.N-ob c' (e , Pc')))) 
                (cong (𝓒 ⋆ id 𝓒)  (𝓒 .⋆IdL _))})})

    module _ {C D : Category ℓ ℓ'}(F : Functor D C) {ℓS ℓE : Level} where 
        open model C {ℓS} renaming (𝓟Mon to 𝓒Mon ; π₂p to Cπ₂p ; _^_ to _^𝓒_ ; self to self𝓒 ; ℓm to ℓm𝓒) 
        open model D {ℓS} renaming (𝓟Mon to 𝓓Mon; _^_ to _^𝓓_ ; self to self𝓓 ; ℓm to ℓm𝓓)
        
        module _ (EC : EnrichedCategory 𝓒Mon ℓE ) where

          open MonoidalCategory 𝓒Mon
            renaming (ob to ob𝓒; Hom[_,_] to 𝓒[_,_]; id to id𝓒; _⋆_ to _⋆𝓒_; unit to unit𝓒; _⊗_ to _⊗𝓒_; η⟨_⟩ to η𝓒⟨_⟩ ; _⊗ₕ_ to _⊗ₕ𝓒_ )
          
          open MonoidalCategory 𝓓Mon
            renaming (ob to ob𝓓; Hom[_,_] to 𝓓[_,_]; id to id𝓓; _⋆_ to _⋆𝓓_; unit to unit𝓓 ; _⊗_ to _⊗𝓓_; η⟨_⟩ to η𝓓⟨_⟩ ; _⊗ₕ_ to _⊗ₕ𝓓_; ρ⟨_⟩ to ρ𝓓⟨_⟩ )
          
          distrib^ : {X Y : ob𝓒} → 𝓓[ reindPsh F (Y ^𝓒 X) , reindPsh F Y ^𝓓 reindPsh F X ] 
          distrib^ .N-ob d exp = 
            pshhom 
            (λ {d' (f , XFd') → exp .PshHom.N-ob (F .F-ob d') (F .F-hom f , XFd')}) 
            λ {d d' f (g , FXd') → cong (λ h → exp .PshHom.N-ob _ h) (cong₂ _,_ (F .F-seq _ _) refl) 
            ∙ exp .PshHom.N-hom (F .F-ob d) (F .F-ob d')  (F .F-hom f) (F .F-hom g , FXd')}
          ...
          distrib : {x y z : ob EC} → 
            𝓓[ reindPsh F EC[ x , y ] ⊗𝓓 reindPsh F EC[ y , z ] , reindPsh F (EC[ x , y ] ⊗𝓒 EC[ y , z ]) ]
          distrib = natTrans (λ _ x → x) λ _ → refl

          Eid : {x : ob EC} → 𝓓[ unit𝓓 , reindPsh F EC[ x , x ] ]
          Eid = const ●ᵛ (EC .id ∘ˡ (F ^opF))

          Eseq : {x y z : ob EC} → 𝓓[ reindPsh F EC[ x , y ] ⊗𝓓 reindPsh F EC[ y , z ] , reindPsh F EC[ x , z ] ]
          ...
          BaseChange .⋆IdL x y = 
            makeNatTransPath (funExt λ d → funExt⁻ (cong (N-ob) (EC .⋆IdL x y)) (F-ob F d))
          BaseChange .⋆IdR x y = 
            makeNatTransPath (funExt λ d → funExt⁻ (cong (N-ob) (EC .⋆IdR x y)) (F-ob F d))
          BaseChange .⋆Assoc x y z w = 
            makeNatTransPath (funExt λ d → funExt⁻ (cong N-ob (EC .⋆Assoc x y z w)) (F-ob F d))
    ...
    module _ {C : Category ℓ ℓ'}{ℓS : Level}  where 
      ...
      module _ (E : EnrichedCategory 𝓟Mon ℓS ) where
        ...
        open EnrichedCategory E renaming (ob to obE ; Hom[_,_] to E[_,_]; id to idE ; seq to seqE ; ⋆IdL to ⋆IdLE ; ⋆IdR to ⋆IdRE ; ⋆Assoc to ⋆AssocE)
        ...
        norm .⋆Assoc f g h = 
          makeNatTransPath (funExt λ c → funExt λ{ tt* →
            λ i → ⋆AssocE _ _ _ _ i .N-ob c (f .N-ob c tt* , (g .N-ob c tt* , h .N-ob c tt*))})
        norm .isSetHom = 𝓟 .isSetHom

    -- Psh(SET) enriched category from a normal category 
    module _ (C : Category ℓ ℓ') where 
      open model (SET ℓ') {ℓ'} renaming (𝓟Mon to 𝓟Set)
      open MonoidalCategory 𝓟Set renaming (Hom[_,_] to 𝓟Set[_,_] ; _⊗_ to _⊗𝓟_ ; η⟨_⟩ to η𝓟⟨_⟩ ; ρ⟨_⟩ to ρ𝓟⟨_⟩ ; _⊗ₕ_ to _⊗ₕ𝓟_) hiding (C)
    ...
    module _ (C D : Category ℓ ℓ') (F : Functor C D) where 
      ...
      enrich-fmap : {c c' : ob (enrich C)} → 𝓟 [ (enrich C) .Hom[_,_] c c' , (enrich D) .Hom[_,_] (F .F-ob c) (F .F-ob c') ]
      enrich-fmap = natTrans (λ A P → lift (λ a → F .F-hom (P .lower a))) λ f → refl
      
      enrichF : EnrichedFunctor 𝓟Set (enrich C) (enrich D) 
      enrichF .F₀ = F .F-ob
      enrichF .F₁ {c}{c'} = enrich-fmap
      enrichF .Fid =  
        makeNatTransPath (funExt λ A →  funExt λ {tt* → cong lift (funExt λ a → F .F-id)})
      enrichF .Fseq = 
        makeNatTransPath (funExt λ A → funExt λ {(f , g) → cong lift (funExt λ a → 
          sym (F .F-seq (f .lower a) (g .lower a) ))})
      
    -- change of base for self enrichment
    module _ {C D : Category ℓ ℓ'}(F : Functor D C)  where 
      open model C {ℓ'} renaming (𝓟Mon to 𝓒Mon ; π₂p to Cπ₂p ; _^_ to _^𝓒_ ; self to self𝓒 ; ℓm to ℓm𝓒) 
      open model D {ℓ'} renaming (𝓟Mon to 𝓓Mon; _^_ to _^𝓓_ ; self to self𝓓 ; ℓm to ℓm𝓓)
      ℓh = ℓ-max ℓ ℓ'
      -- note ℓm𝓒 = ℓh = ℓm𝓓
      ...
      BaseChangeSelf .Fid = makeNatTransPath (funExt λ m → funExt λ {tt* → makePshHomPath (funExt λ n → funExt λ {(f , XFn) → refl})})
      BaseChangeSelf .Fseq = makeNatTransPath (funExt λ m → funExt λ{(f , x) → makePshHomPath (funExt λ n → funExt λ {(g , XFn) → 
          cong (λ h → x . PshHom.N-ob _ h) 
              (cong₂ _,_ 
                  (cong (λ h → F .F-hom h) (D .⋆IdL _) ∙ sym (C .⋆IdL _)) 
                  (cong (λ h → f .PshHom.N-ob _ (h , XFn)) (cong (λ h → F .F-hom h) (D .⋆IdL _) ∙ sym (C .⋆IdL _))))})})
    ...
    module _ 
        ...
        module _ 
            ...
            BaseChangeF .Fid = makeNatTransPath (funExt λ d → funExt⁻ (cong N-ob (Fid 𝓖)) (F-ob F d))
            BaseChangeF .Fseq = makeNatTransPath (funExt λ d → funExt⁻ (cong N-ob (Fseq 𝓖)) (F-ob F d))
