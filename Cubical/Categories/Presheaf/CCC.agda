...
module _ (C : Category ℓ ℓ') (ℓS : Level) where
  ...
  ExpOb A B .F-ob c =
    (𝓟 [ PshProd ⟅ LiftF {ℓ'}{ℓm} ∘F (YONEDA .F-ob c) , A ⟆b , B ]) ,
    𝓟 .isSetHom
  ExpOb A B .F-hom {X}{Y} Y→X M =
    (PshProd .Bif-hom× (LiftF {ℓ'}{ℓm} ∘ʳ (YONEDA .F-hom Y→X)) (𝓟 .id)) ⋆⟨ 𝓟 ⟩ M
  ExpOb A B .F-id =
    funExt λ M →
      makeNatTransPath (
        funExt λ Z →
  ...
  ⇒𝓟 : Exponentials 𝓟 (×𝓟 C (ℓ-max (ℓ-max ℓ ℓ') ℓS))
  ⇒𝓟 (A , B) .vertex = ExpOb B A
  ⇒𝓟 (A , B) .element = eval A B
  ⇒𝓟 (A , B) .universal Z .equiv-proof Z×B→A = uniqueExists
    (natTrans (λ x Zx → natTrans
      (λ{y (y→x , By) → Z×B→A .N-ob y (Z .F-hom (y→x .lower) Zx , By)})
      λ{y}{z}z→y → funExt λ{ (y→x , By) →
        cong (λ h → Z×B→A .N-ob z (h , B .F-hom z→y By ))
        (funExt⁻ (Z .F-seq _ _ ) Zx)
        ∙ funExt⁻ (Z×B→A .N-hom z→y) (Z .F-hom (y→x .lower) Zx , By)  })
      λ{x}{y}f → funExt λ Zx → makeNatTransPath
        (funExt λ z → funExt λ{(y→z , Bz)→ cong (λ h → Z×B→A .N-ob z (h , Bz))
