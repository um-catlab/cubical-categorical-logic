...
module _ (Σ₀ : hGroupoid ℓ) where
  ...
  lookupPVar : {X : ⟨ Σ₀ ⟩ → Type ℓ'} → ListP X Γ → Var Γ A → X A
  lookupPVar (y ∷ Γ) vz = y
  lookupPVar (y ∷ Γ) (vs x) = lookupPVar Γ x
  ...
  FreeSCwF .SCwF.comprehension = CtxExt
  ...
  module _ (M : SCwFᴰ FreeSCwF ℓC ℓC' ℓT ℓT') where
    ...
    module _ (ı : ∀ A → M.Tyᴰ A) where
      ...
      elimSection : GlobalSection M.Cᴰ
      ...
      elim .StrictSection.F = elimSection
      elim .StrictSection.F-ty = ı
      elim .StrictSection.F-tm = λ _ → elimPshSection
      elim .StrictSection.F-term = refl
      elim .StrictSection.F-comp A Γ =
        ΣPathP (refl , (ΣPathP
