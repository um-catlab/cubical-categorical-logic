...
module Cubical.Categories.WithFamilies.Simple.Instances.Free.Renaming where
...
module _ (Σ₀ : hGroupoid ℓ) where
  ...
  FreeSCwF .SCwF.C = RenamingCategory
  FreeSCwF .SCwF.Ty = ⟨ Σ₀ ⟩
  FreeSCwF .SCwF.Tm = VarPsh
  FreeSCwF .SCwF.term = TermCtx
  FreeSCwF .SCwF.ext = CtxExt
  ...
  module _ (M : SCwFᴰ FreeSCwF ℓC ℓC' ℓT ℓT') where
    ...
    module _ (ı : ∀ A → M.Tyᴰ A) where
      ...
      elimS-F-ob (A ∷ Γ) = M.extᴰObᴰ (ı A) (elimS-F-ob Γ)

      elimVar : ∀ {Γ A} (x : Var Γ A) → M.Tmᴰ[ x ][ elimS-F-ob Γ , ı A ]
      elimVar {Γ = A ∷ Γ}{A = A} vz = M.extᴰ.elementᴰ .snd
      elimVar (vs x) = M.Tmᴰ.reind lemma $ (M.extᴰ.elementᴰ .fst M.Tmᴰ.⋆ᴰ elimVar x) -- M.Tmᴰ.reind lemma $ (M.extᴰ.elementᴰ .fst) M.Tmᴰ.⋆ᴰ elimVar x
        where
      ...
      -- This got nastier with uncurried presheaves...
      elimRenWkRen : ∀ {Δ Γ A}(γ : Renaming Δ Γ)
        → elimRen (wkRen {A = A} γ) M.Cᴰ.∫≡ (M.extᴰ.elementᴰ .fst M.Cᴰ.⋆ᴰ elimRen γ)
      elimRenWkRen [] = M.termᴰ.introᴰ≡ refl
      elimRenWkRen (x ∷ γ) = M.extᴰ.introᴰ≡ (ΣPathPᴰ (elimRenWkRen γ ∙ M.Cᴰ.⟨⟩⋆⟨ (sym $ PathPᴰΣ (M.extᴰ.∫βᴰ _) .fst) ∙ (sym $ M.Cᴰ.reind-filler _ _) ⟩ ∙ sym (M.Cᴰ.⋆Assoc _ _ _) ∙ M.Cᴰ.reind-filler _ _)
        ((sym $ M.Tmᴰ.reind-filler _)
        ∙ M.Tmᴰ.⟨⟩⋆⟨ (sym $ PathPᴰΣ (M.extᴰ.∫βᴰ (elimRen γ , elimVar x)) .snd) ∙ M.Tmᴰ.⋆ᴰ-reind _ _ _ ⟩ ∙  ((sym $ M.Tmᴰ.⋆Assoc _ _ _)) ∙ sym (M.Tmᴰ.⋆ᴰ-reind _ _ _)))

      elimRen-Var : ∀ {Δ Γ A}
        → (γ : Renaming Δ Γ)
        → (x : Var Γ A)
        → elimVar (ren γ x) M.Tmᴰ.∫≡ (elimRen γ M.Tmᴰ.⋆ᴰ elimVar x)
      elimRen-Var (y ∷ γ) vz = (sym $ PathPᴰΣ (M.extᴰ.∫βᴰ _) .snd) ∙ {!!}
