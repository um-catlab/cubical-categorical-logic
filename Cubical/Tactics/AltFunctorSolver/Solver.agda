...
module Eval (𝓒 : Category ℓc ℓc') (𝓓 : Category ℓd ℓd')  (𝓕 : Functor 𝓒 𝓓) where
  ...
  sem𝓒 : GlobalSection (weaken Free𝓒 𝓒)
  sem𝓒 = FreeCat.elim (Cat→Quiver 𝓒) _ ı where
    ı : Interpᴰ _ _ _
    ı $gᴰ u = u
    ı <$g>ᴰ e = e .snd .snd

  sem𝓓 : GlobalSection (weaken Free𝓓 𝓓)
  ...
  Y : GlobalSection (weaken Free𝓓 𝓟F𝓓)
  Y = Weaken.intro IdF (PseudoYoneda {C = Free𝓓})

  module Y = Section Y
  open HetQG

  selfFree𝓒 : GlobalSection (weaken Free𝓒 𝓟F𝓓)
  selfFree𝓒 = FreeCat.elim (Cat→Quiver 𝓒) _ ı where
    ı : Interpᴰ _ _ _
    ı ._$gᴰ_ = (PseudoYoneda {C = Free𝓓} ∘F Self.FreeFunctor) .F-ob
    ı <$g>ᴰ e = (PseudoYoneda {C = Free𝓓} ∘F Self.FreeFunctor) .F-hom (η𝓒 <$g> e)

  Normalize : GlobalSection (weaken Free𝓓 𝓟F𝓓)
  Normalize =
    Self.elim selfFree𝓒 (weakenF IdF) (λ A → Y.F-obᴰ (inr A)) ı where
    ı : ∀ (e : DGen) → _
    ı (inl x , inl x₁ , e) =
      Y.F-homᴰ (Self.moduloAx ⟪ η𝓓 <$g> (inr (_ , _ , e)) ⟫)
    ı (inl x , inr x₁ , e) =
      Y.F-homᴰ (Self.moduloAx ⟪ η𝓓 <$g> (inr (_ , _ , e)) ⟫)
    ı (inr x , inl x₁ , e) =
      Y.F-homᴰ (Self.moduloAx ⟪ η𝓓 <$g> (inr (_ , _ , e)) ⟫)
  ...
  Normalize≡Y = PathReflection _ Normalize Y N≡Y where
    N≡Y : Section _ _
    N≡Y = {!Self.elim!}
  --               (weaken Free𝓓 𝓟F𝓓)
