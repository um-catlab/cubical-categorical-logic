...
module Dynamics.CBPVSmallStep where 
    ...
    module mod {ℓ : Level} where 
        ...
        semcmp S .F-ob Γ = 
            (Lift (set [ Γ , S .state ])) , isOfHLevelLift 2 (set .isSetHom {Γ}{S .state})
        semcmp S .F-hom γ m = lift (m .lower ∘S γ)
        semcmp S .F-id = refl 
        semcmp S .F-seq _ _ = refl 

        stacktrans : {S T : TSystem {ℓ}} → NatTrans E[ S , T ] self[ semcmp S , semcmp T ]
        ...
        sem .vTm A = set [-, A ]
        sem .cTm = EF
        sem .emp = (Unit*  {ℓ}, isSetUnit*) , λ X → (λ _ → tt*) , λ f → funExt λ _ → refl
        sem ._×c_ = λ A B → ⟨ A ⟩ × ⟨ B ⟩ , isSetΣ (A .snd) λ _ → B .snd
        sem .up×c Γ A = 
            record { 
                trans = natTrans (λ X f → (λ x → fst (f x)) , (λ x → snd (f x))) λ _ → refl ; 
                nIso = λ X → 
                isiso 
                    (λ {(f , g) x → (f x) , (g x)}) 
                    (funExt (λ _ → refl)) 
