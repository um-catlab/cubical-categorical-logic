...
module Dynamics.TransitionSystem where 
  ...
  module TSys {ℓ : Level} where 
    ...
    map-Maybe-seq : {A B C : Set ℓ}{f : A → B}{g : B → C} → (x : Maybe A) → map-Maybe (λ x₂ → g (f x₂)) x ≡ map-Maybe g (map-Maybe f x)
    ...
    ≡-to-≤m  : {A : hSet ℓ}{x y : ⟨ A ⟩}{m n : Maybe ⟨ A ⟩ } → m ≡ just x → n ≡ just y → x ≡ y → _≤m_ {A} m n 
    ≡-to-≤m {A} p q r = subst2 (λ h1 h2 → _≤m_ {A} h1 h2) (sym p) (sym q) r

    eq : {A : hSet ℓ }{m n : Maybe ⟨ A ⟩} → m ≡ n → _≤m_ {A} m n 
    eq {A} {nothing} {n} _ = tt*
    eq {A} {just x} {nothing} p = ⊥elim {A = λ _ → _≤m_{A} (just x) nothing}(¬nothing≡just (sym p))
    eq {A} {just x} {just x₁} p = just-inj _ _ p

    inversion : {A : hSet ℓ}{a : ⟨ A ⟩ }{n : Maybe ⟨ A ⟩ } → _≤m_ {A} (just a) n → Σ ⟨ A ⟩ λ a' → (n ≡ just a') × (a ≡ a' )
    ...
    ≤m-trans : {A : hSet ℓ}{m n p : Maybe ⟨ A ⟩ } → _≤m_{A} m n → _≤m_ {A} n p → _≤m_ {A} m p 
    ...
    monoF : {A B : hSet ℓ}{x y : Maybe ⟨ A ⟩ }{f : ⟨ A ⟩  → ⟨ B ⟩ } → _≤m_ {A} x y → _≤m_ {B} (map-Maybe f x) (map-Maybe f y) 
    monoF {A} {B} {nothing} {y} p = tt*
    monoF {A} {B} {just x} {just y}{f} p = cong f p

    monoF' : {A B : hSet ℓ}{x y : Maybe ⟨ A ⟩ }{f g : ⟨ A ⟩  → ⟨ B ⟩ } → f ≡ g →  _≤m_ {B} (map-Maybe f x) (map-Maybe g x) 
    monoF' {A} {B} {nothing} {y} {f} {g} p = tt*
    monoF' {A} {B} {just x} {y} {f} {g} p = funExt⁻ p _

    monoBind : {A B C : hSet ℓ}{x : Maybe ⟨ A ⟩ }{f : ⟨ A ⟩  → ⟨ B ⟩ }{g : ⟨ B ⟩ → ⟨ C ⟩ }
    ...
    maybePreorder X = (Maybe ⟨ X ⟩  , 
        preorderstr (_≤m_{X})
          (ispreorder (λ a b → ≤m-isProp) (λ a → ≤m-refl) λ a b c → ≤m-trans)) , 
          isSetMaybe {X}
    ...
    MaybeOrdered = record { F = MaybeF ; ≤ = ord ; commute = Functor≡ (λ _ → refl) (λ _ → refl)}
    ...
    record TSystem : Set(ℓ-suc ℓ) where 
      ...
      hterm : hSet _ 
      hterm = term , isSetΣ (state .snd) λ _ → isOfHLevelSuc 1 ((isSetMaybe {state} _ _))
      ...
      ¬-fiber→nothing : (s : ⟨ state ⟩ ) → ¬ (Σ ⟨ state ⟩ λ s' → trans s ≡ just s') → trans s ≡ nothing
      ...
      ... | inr (s' , prf') = ⊥elim {ℓ}{λ _ → inr (s' , prf') ≡ inl prf} ((¬nothing≡just (sym prf ∙ prf')))

      -- things that can step, always step
      willStep : ((s , (s' , prf)) : steps) → canStep? s ≡ inr (s' , prf)
      willStep (s , s' , prf) with canStep? s 
      ... | inl x = ⊥elim {ℓ} {λ _ → inl x ≡ inr (s' , prf)} (¬nothing≡just  (sym x ∙ prf))
      ... | inr (s'' , prf') = cong inr (ΣPathP (goal , toPathP (isSetMaybe {state} _ _ _ _))) where 
        goal : s''  ≡ s'
    ...
    module _ (S T : TSystem) where 
      record TSystem[_,_] : Set ℓ where 
        field 
          ...
          comm : {s : ⟨ S .state ⟩ } →  _≤m_{T .state} (map-Maybe tmap (S .trans s))  (T .trans (tmap s))

      TSystemHomSigma : Type ℓ 
      TSystemHomSigma = 
        Σ (⟨ S .state ⟩  → ⟨ T .state ⟩) 
          λ f → {s : ⟨ S .state ⟩ } → _≤m_ {T .state}(map-Maybe f (S .trans s) ) (T .trans (f s))

      TSysHomIsoΣ : Iso (TSystem[_,_]) (TSystemHomSigma)
      unquoteDef TSysHomIsoΣ = defineRecordIsoΣ TSysHomIsoΣ (quote (TSystem[_,_]))

    open TSystem[_,_]


    module _ {S T : TSystem}(f  : TSystem[ S , T ])((s , (s' , s↦s')) : steps S) where 
      {-
          s --tmap f --> f(s)
          | S trans      | trans T
          |              |
      ...
      step-T : Σ ⟨ T .state ⟩ (λ a' → (T .trans (tmap f s) ≡ just a') × (f .tmap s' ≡ a'))
      step-T = inversion (subst ( λ h → h ≤m T .trans (tmap f s) ) (cong₂ map-Maybe refl s↦s') (f .comm))

      commutes : T .trans (tmap f s) ≡ just (tmap f s')
      commutes = step-T  .snd .fst ∙ cong just (sym (step-T .snd .snd))
  
    TSysMap≡ : {S T : TSystem}{f g : TSystem[ S , T ]} → f .tmap ≡ g .tmap → f ≡ g 
    TSysMap≡ {S}{T}{f}{g} p = isoFunInjective (TSysHomIsoΣ S T) f g (Σ≡Prop (λ f → isPropImplicitΠ λ _ → ≤m-isProp) p)

    open Iso
    TSysMapisSet : {S T : TSystem} → isSet (TSystem[ S , T ])
    TSysMapisSet {S} {T} = isSetRetract  (fun (TSysHomIsoΣ S T)) ((inv (TSysHomIsoΣ S T))) ((leftInv (TSysHomIsoΣ S T))) 
      (isSetΣ (isSet→ (T .state .snd)) λ _ → isProp→isSet (isPropImplicitΠ λ _ → ≤m-isProp))

    idSysHom : {S : TSystem} → TSystem[ S , S ]
    idSysHom .tmap s = s
    idSysHom {S} .comm {s} = subst (λ x → x ≤m S .trans s) (sym (map-Maybe-id _)) ≤m-refl 

    _∘TS_ : {S T R : TSystem} → TSystem[ S , T ] → TSystem[ T , R ] → TSystem[ S , R ]
    _∘TS_ {S}{T}{R} f g .tmap = g .tmap ∘S f .tmap
    _∘TS_ {S}{T}{R} f g .comm {s} = ≤m-trans (≤m-trans (monoBind{S .state}{T .state}) (monoF (f .comm))) (g .comm)
