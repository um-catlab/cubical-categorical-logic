...
module Gluing.CBPV.Model where 
    ...
    record CBPVModel {ℓ ℓ' ℓS ℓE : Level} : Type (ℓ-suc (ℓ-suc (ℓ-max ℓ (ℓ-max ℓ' (ℓ-max ℓS ℓE))))) where 
        field 
            𝓒 : Category ℓ ℓ' 
        ...
        field
            ...
            emp : Terminal 𝓒
            _×c_ : ob 𝓒 → vTy  → ob 𝓒
            up×c : (Γ : ob 𝓒)(A : vTy ) → 𝓒 [-, (Γ ×c A) ]  ≅ᶜ (𝓒 [-, Γ ]) ×Psh vTm A 
          
    record CBPVModelHom {ℓ ℓ' : Level} (M N : CBPVModel{ℓ}{ℓ'}) : Type (ℓ-suc (ℓ-suc (ℓ-max ℓ ℓ'))) where 
        ...
        adjust = ecomp mod𝓒.𝓟Mon stk (ecomp mod𝓒.𝓟Mon (BaseChangeF ctx N.cTm) (BaseChangeSelf ctx) )
        field 
    ...
    module InitialModel {ℓ : Level} where 
        ...
        E .seq _ _ _ = natTrans (λ{x₁ (k , k') → k' ∘k k}) λ _ → funExt λ _ →  sym substDist
        E .⋆IdL _ _ = makeNatTransPath (funExt λ _ → funExt λ _  → sym ∘kIdR)
        E .⋆IdR _ _ = makeNatTransPath (funExt λ _ → funExt λ _  → sym ∘kIdL)
        E .⋆Assoc _ _ _ _ = makeNatTransPath  (funExt λ _ → funExt λ _ →  ∘kAssoc)
        ...
        plug B B' .N-hom γ = funExt λ k → 
          makePshHomPath (funExt λ Θ → funExt λ (δ , m) → cong (λ h → h [ m ]∙ ) (sym subAssocK))
        ...
        ctm .Fseq = 
          makeNatTransPath (funExt λ Γ → funExt λ (k , k') → 
            makePshHomPath (funExt λ Δ → funExt λ (γ , m) → 
               cong₂ (λ h1 h2 → ((k' [ h1 ]k) [ (k [ h2 ]k) [ m ]∙ ]∙)) ∘sIdR ∘sIdR 
               ∙ sym plugAssoc
               ∙ cong (λ h → ( h [ m ]∙)) (sym substDist)))

        open NatIso
        up : (Γ : Ctx) (A : VTy) → SCat [-, (A ∷ Γ) ] ≅ᶜ ((SCat [-, Γ ]) ×Psh vtm A) 
