...
module Gluing.CartesianClosedCategory.Conservativity.OverCategories.Base where
...
module _ (Q : Quiver ℓQ ℓQ') where
  ...
  FREE-1,×,⇒ = FCCC.FreeCartesianClosedCategory ×⇒Q
  ...
  extension = FCCC.rec ×⇒Q (CCC-PRESHEAF FREE (ℓ-max ℓQ ℓQ'))
    (interpᴰ (YOStrict ⟅_⟆) λ f → YOStrict ⟪ (↑ f) ⟫)
  ...
  S = FCCC.elimLocal ×⇒Q (nerve , nerve-pres-bp) PSHᴰCartesianClosedⱽ
    ...
    HOM : (e : Q.mor) →
      PSHᴰ.Hom[ nerve ⟪ ⊆ ⟪ (↑ e) ⟫ ⟫ ][ OB (Q.dom e) , OB (Q.cod e) ]
    HOM e .N-ob (o , _ , f) (g , p) =
      (↑ e) ∘⟨ FREE ⟩ g ,
      ⊆ .F-seq g (↑ e) ∙ cong (λ x → x ⋆⟨ FREE-1,×,⇒.C ⟩ ⊆ ⟪ (↑ e) ⟫) p
