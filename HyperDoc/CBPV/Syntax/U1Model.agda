...
module HyperDoc.CBPV.Syntax.U1Model where
...
module Syntax (T : Theory) where 
  ...
  data _⊢c_ where 
    ...
    eqns : ∀{A : VTy}{B : CTy} 
      (e : Eq)
      (γ : Fin (ctx (ax e)) → A ⊢c B) → 
       eval {!   !} (lhs (ax e)) γ ≡ eval {! satisfies  !} (rhs (ax e)) γ
{-
(e : Eq) (ρ : Fin (Equation.ctx (ax e)) → A ⊢c B) →
eval (FreeCompAlg A B) (Equation.lhs (ax e)) ρ ≡
eval (FreeCompAlg A B) (Equation.rhs (ax e)) ρ
-}
    -- type structure
    force : {B : CTy} → U B ⊢c B      
    yes : 𝟙 ⊢c Ans 
...
module SyntacticModel (T : Theory)  where 
  open Theory T
  open Syntax T
  ...
  FreeCompAlg : VTy → CTy → Alg Sig
  FreeCompAlg A B .Carrier = A ⊢c B , isSet⊢c
  FreeCompAlg A B .interp = ops A B

  FreeCompModel : VTy → CTy → Model T
  FreeCompModel A B .Model.alg = FreeCompAlg A B
  FreeCompModel A B .Model.sound = {!   !}
  
  O : Functor (V ^op ×C C) (MOD T) 
  O .F-ob (A , B) .fst = FreeCompAlg A B
  O .F-ob (A , B) .snd e env = {! eval (FreeCompAlg A B) (lhs (ax e)) env  !}
